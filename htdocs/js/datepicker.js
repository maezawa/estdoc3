...
calendar = {
	...
	makeCalendar:function(year, month, day) {
		...
		this.wrt("<th colspan='5' class='calendar-title'>");
		...
		for(var i=0;i<=5;i++) {
			...
			for(var j=0;j<7;j++) {
				...
				if(flag) {
					...
					var dt1 = new Date(yea, this.today.getMonth() + 1, this.today.getDate());
					var dt2 = new Date(year, mon-0 , w);
					var diff = parseInt((dt2 - dt1) / 86400000);

					this.wrt("<td class='days"+class_name+"'><a class=\"filter\" data-date=\"" + diff + "\">"+w+"</a></td>");
					d++;
				} else {
		...
		this.wrt("</table>");
		this.wrt("<input type='button' value='閉じる' class='button close calendar-cancel' onclick='calendar.hideCalendar();' />");
	...
	showCalendar: function() {
		...
		div.style.left = (xy[0] - width + 92) + "px";
		div.style.top = (xy[1] - 240) + "px";
